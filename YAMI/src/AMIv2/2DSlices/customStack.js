 ...
 * changes :
 *   - pack does a new kind of texture (2D slice raw data)
 * */
...
export default class ModelsStack extends ModelsBase {
  ...
  copy_values(stack) {
    this._uid = stack._uid;
    this._stackID = stack._stackID;

    this._frame = stack._frame;
    this._numberOfFrames = stack._numberOfFrames;

    this._rows = stack._rows;
    this._columns = stack._columns;
    this._numberOfChannels = stack._numberOfChannels;
    this._bitsAllocated = stack._bitsAllocated;
    ...
    this._rescaleSlope = stack._rescaleSlope;
    this._rescaleIntercept = stack._rescaleIntercept;

    this._minMax = stack._minMax;

    // TRANSFORMATION MATRICES
    this._regMatrix = stack._regMatrix;

    this._ijk2LPS = stack._ijk2LPS;
    this._lps2IJK = stack._lps2IJK;

    this._aabb2LPS = stack._aabb2LPS;
    this._lps2AABB = stack._lps2AABB;

    //
    // IJK dimensions
    this._dimensionsIJK = stack._dimensionsIJK;
    this._halfDimensionsIJK = stack._halfDimensionsIJK;
    this._spacing = stack._spacing;
    this._spacingBetweenSlices = stack._spacingBetweenSlices;
    this._sliceThickness = stack._sliceThickness;
    this._origin = stack._origin;
    this._rightHanded = stack._rightHanded;
    this._xCosine = stack._xCosine;
    this._yCosine = stack._yCosine;
    this._zCosine = stack._zCosine;
    ...
    //
    this._modality = stack._modality;
  ...
  prepareSegmentation() {
    ...
    for (let i = 0; i < this._frame.length; i++) {
      ...
      mergedFrames[prevIndex].minMax =
        AMI.UtilsCore.minMax(mergedFrames[prevIndex]._pixelData);
    }
    ...
    for (let i = 0; i < this._segmentationSegments.length; i++) {
      ...
      if (color === null) {
        ...
        dict[this._segmentationSegments[i].segmentNumber] =
          AMI.CoreColors.cielab2RGB(...color);
      }
    }
  ...
  prepare() {
    ...
    this._minMax[0] = AMI.UtilsCore.rescaleSlopeIntercept(
      this._minMax[0],
      this._rescaleSlope,
      this._rescaleIntercept);
    this._minMax[1] = AMI.UtilsCore.rescaleSlopeIntercept(
      this._minMax[1],
      this._rescaleSlope,
      this._rescaleIntercept);
  ...
  computeIJK2LPS() {
    // ijk to lps
    this._ijk2LPS = AMI.UtilsCore.ijk2LPS(
   ...
   */
  computeLPS2AABB() {
    this._aabb2LPS = AMI.UtilsCore.aabb2LPS(
      this._xCosine, this._yCosine, this._zCosine,
      this._origin
    );
  ...
  static value(stack, coordinate) {
    ...
    return AMI.UtilsCore.value(stack, coordinate);
  }
  ...
  static worldToData(stack, worldCoordinates) {
    ...
    return AMI.UtilsCore.worldToData(stack._lps2IJK, worldCoordinates);
  }
}
